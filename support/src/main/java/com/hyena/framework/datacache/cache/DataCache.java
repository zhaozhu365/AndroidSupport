...
public class DataCache {
	...
	public Cacheable get(String key, CacheEntry entry) throws CacheExpiredException, CacheUncachedException {
		...
		LogUtil.d(TAG, "readFromMemCache entry : " + cacheEntry);
		if (cacheEntry == null)
	 ...
	 *            缓存对象
	 * @param validTime
	 *            缓存时间(以毫秒计，超过该时间后将无法从缓存模块获得该缓存对象)
	 * @return 缓存成功后的缓存对象
	 */
	...
	private void putIntoMemCache(String key, CacheEntry entry) {
		...
		LogUtil.d(TAG, "putIntoMemCache entry : " + entry);
	...
	private void validate() {
		LogUtil.d("validate() mCacheSpace : " + mCacheSpace);
