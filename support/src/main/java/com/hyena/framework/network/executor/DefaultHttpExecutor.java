			...
			if(isGet){//Get请求
				...
				if(params != null && params.mOsHandler != null){
					...
					if(params != null && params.mByteFileMap != null) {
						if(params != null && params.mParams != null){
							for (NameValuePair param : params.mParams) {
						...
						//简单提交Post请求
						if(params != null && params.mParams != null){
							entity = new UrlEncodedFormEntity(params.mParams, HTTP.UTF_8);
						}
					}
				}
...
//				request.addHeader("Host", host);
			...
			//添加默认代理
			HttpHost proxy = networkSensor.getProxyHost(url, params.isProxy);
